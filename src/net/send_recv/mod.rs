...
fn _sendto_any(
    ...
    match addr {
        ...
        #[cfg(target_os = "linux")]
        SocketAddrAny::Netlink(nl) => backend::net::syscalls::sendto_netlink(fd, buf, flags, nl),
    }
}
...
#[inline]
#[doc(alias = "sendto")]
pub fn sendto_netlink<Fd: AsFd>(
    fd: Fd,
    buf: &[u8],
    flags: SendFlags,
    addr: &SocketAddrNetlink,
) -> io::Result<usize> {
    backend::net::syscalls::sendto_netlink(fd.as_fd(), buf, flags, addr)
}
